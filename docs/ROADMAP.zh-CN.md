...
这份路线图最初写于 v1.5 结构化升级阶段，当时要解决的是：经验写死在 prompt、benchmark 缺少统一吸收流程、风格分流不稳定，以及动态交互语言还没有形成成体系的观察与验收方法。

这些基础问题现在已经大体补齐：结构化知识层、style option 分流、motion 观察维度、动态验证模板和最终验收链路都已经并入主线。

因此，这份文档现在主要承担两件事：

- 保留从 v1.5 到当前主线的演进脉络
- 继续说明后续 v2 / v2-lite 应该往哪里推进
...
6. 动态交互知识层与验收链路
   - 为 benchmark 增加 scroll / hover / sticky / state transition 观察维度
   - 为每个 option 增加动态边界，区分“克制点缀”和“交互骨架”
   - 用 Playwright 的状态采样来验证行为，而不只看静态截图

#### v2-lite：motion-mvp

在完整 v2 之前，可以先落一个轻量的 `motion-mvp`：

- 不新增 motion 专用 style option
- 不绑定 GSAP / Motion / WAAPI 等具体实现库
- 只补三件事：
  - motion 观察维度
  - option 动态边界
  - Playwright 动态状态采样验收

这个小阶段的目标不是做自动化生成器，而是先让 skill 形成“能看懂动态语言，也能验收动态语言”的最小闭环。
...
- 当参考站的差异主要来自动态交互语言时，skill 也能稳定判断它是在补强旧风格，还是在挑战旧分类
