...
当前主线已经可以按 `v1.7.x` 理解：它不再只是静态页面风格包，而是由风格分流、benchmark 学习，以及第一层动态交互观察与验收链路共同组成。
...
- `templates/`
  - 新 benchmark、新 option 与 motion 分类的记录模板

如果某个 benchmark 的高级感明显依赖滚动、hover、sticky、reveal 或 click 后状态变化，不能只看首屏截图；要把动态语言也纳入结构化判断。
...
以下场景属于“直接执行优先”，不要先走 visual companion 或额外提案确认：

- 用户正在验证这个 skill 的默认行为
- 用户已经明确要成品图、PNG、页面截图、单页模版或可交付页面
- 用户已经显式指定了 style option，且不存在会实质改变结果的关键歧义

在这些场景里：

- 简短说明将采用的默认或指定方向
- 除非存在会显著改变结果的缺失决策，否则不要追加确认
- 直接开始产出目标 artifact

如果上层通用流程与这条规则冲突，以这里的“直接执行优先”为准。
...
- 把首屏当海报，不要当课程目录
- 高级感不一定来自暗场或大片，也可以来自明亮底色、强主张、关键数字和可核验的证据组织
- 内容页不能掉回普通 CMS 模板
- 图像必须承担叙事职责，而不是只做装饰
- 数据和元信息要像任务简报，不要像营销 badge
- 脚注、方法说明、PDF 报告和白皮书入口也要被当成编辑系统的一部分，而不是事后补丁
